import Vinyl from "./Vinyl"
import Record from "./Record"

const recordBin = [
  (new Vinyl(
    "Boys & Girls",
    'Alabama Shakes"',
    36,
    2011,
    ["blues", "garage rock"],
    "Hold On",
    "https://www.youtube.com/watch?v=Le-3MIBxQTw"
  ).isGatefold = true),
  new Vinyl(
    "Sound & Color",
    "Alabama Shakes",
    47,
    2015,
    ["psychedelic, soul"],
    "Don't Wanna Fight",
    "https://www.youtube.com/watch?v=nin-fiNz50M",
    "clear",
    33,
    true,
    2
  ),
  new Vinyl(
    "Fetch the Bolt Cutters",
    "Fiona Apple",
    52,
    2020,
    ["Alternative", "Indie"],
    "Shameika",
    "https://www.youtube.com/watch?v=MbkugCPqxQY",
    "magenta",
    true,
    2
  ),
  new Vinyl(
    "Mostly Improvised Instrumental Indie Music",
    "Renier Bass",
    26,
    2012,
    ["jazz"],
    "Off the Sauce",
    "https://www.youtube.com/watch?v=UuMRUcavgIg",
    "coke bottle green"
  ),
  (new Vinyl(
    "The Lemon of Pink",
    "The Books",
    37,
    2003,
    ["found sounds", "folk", "indie"],
    "PS",
    "https://www.youtube.com/watch?v=XsYEr5GPZzk",
    "pink and yellow starburst"
  ).isGatefold = true),
  new Vinyl(
    "Wobbly Dance Flower",
    "John Daversa",
    41,
    2017,
    ["jazz"],
    "You Got a Puppy?!",
    "https://www.youtube.com/watch?v=UnSKgpKjobc"
  ),
  new Vinyl(
    "Let it Die",
    "Feist",
    45,
    2004,
    ["indie", "folk"],
    "Mushaboom",
    "https://www.youtube.com/watch?v=cYF0qU5WSew",
    "clear seafoam",
    33,
    true
  ),
  new Vinyl(
    "Lianne La Havas",
    "Lianne La Havas",
    52,
    2020,
    ["neo soul"],
    "Weird Fishes",
    "https://www.youtube.com/watch?v=LdbHO_KhCig",
    "clear"
  ),
  new Vinyl(
    "Jaimie",
    "Britanny Howard",
    35,
    2019,
    ["blues", "rock", "soul", "psychedelic"],
    "13th Century Metal",
    "https://www.youtube.com/watch?v=TQoVOAe2o9s",
    "clear"
  ),
  new Record(
    "American Dream",
    "LCD Soundsystem",
    69,
    2017,
    ["dance", "rock"],
    "Call the Police",
    "https://www.youtube.com/watch?v=EW3av1qgk00",
    "casette",
    "clear blue"
  ),
  new Vinyl(
    "Semper Femina",
    "Laura Marling",
    42,
    2016,
    ["folk", "rock"],
    "Nothing, Not Nearly",
    "https://www.youtube.com/watch?v=3cZ1_IcPoN4",
    "black",
    33,
    true,
    2
  ),
  new Vinyl(
    "All Lives You Say?",
    "Wilco, The Punch Brothers",
    6,
    2020,
    ["folk", "indie"],
    "All Lives You Say?",
    "https://www.youtube.com/watch?v=kEmBTmEl1bI",
    "clear pink",
    45
  ),
  new Vinyl(
    "Ultimate Success Today",
    "Protomartyr",
    40,
    2020,
    ["post punk"],
    "Processed By The Boys",
    "https://www.youtube.com/watch?v=yBQy_S_k-qg",
    "clear blue"
  ),
  (new Vinyl(
    "American Egypt",
    "Mike Viola",
    40,
    2018,
    ["alternative"],
    "King Kong Hand",
    "https://www.youtube.com/watch?v=v3nb8Tmk6jM"
  ).isGatefold = true),
  new Record(
    "Red Earth",
    "Y La Bamba",
    36,
    2006,
    ["alternative"],
    "Red Earth",
    "https://www.youtube.com/watch?v=2M6MEU_ZqVA",
    "cassette"
  ),
  new Record(
    "Oh My God",
    "Kevin Morby",
    50,
    2019,
    ["alternative"],
    "Piss River",
    "https://www.youtube.com/watch?v=NjN-aKAmkEg",
    "cassette",
    "white"
  ),
  new Record(
    "1989",
    "Ryan Adams",
    54,
    2015,
    ["rock", "pop"],
    "STYLE",
    "https://www.youtube.com/watch?v=JSyus3HlQPI",
    "cassette",
    "clear seafoam"
  ),
  new Record(
    "No Romance",
    "Greyson Elkins",
    30,
    2017,
    ["indie", "alternative"],
    "No Romance Being Haunted",
    "https://soundcloud.com/greysonelkins/no-romance-being-haunted",
    "cassette",
    "clear"
  ),
  (new Vinyl(
    "Stairway to Paradise",
    "Mike Viola",
    15,
    2016,
    ["rock"],
    "Stairway to Paradise",
    "https://www.youtube.com/watch?v=Qyi2r3C2-pU"
  ).speed = 45),
  new Vinyl(
    "Every Stone",
    "Manchester Orchestra",
    "https://www.youtube.com/watch?v=iqNRVew_5Jw",
    10,
    2014,
    ["rock", "emo", "post-hardcore"],
    "Escape",
    ""
  ),
  (new Vinyl(
    "Alone on Christmas Day",
    "Phoenix, Bill Murray",
    10,
    2015,
    ["indie", "christmas"],
    "Alone on Christmas Day",
    "https://www.youtube.com/watch?v=UXnuXLNmr20"
  ).speed = 45),
  new Vinyl(
    "Smooth Jazz Apocalypse",
    "Reinier Baas",
    10,
    2018,
    ["jazz"],
    "https://www.youtube.com/watch?v=AYAApV0f4Ak",
    "Smooth Jazz Apocalypse",
    "clear yellow",
    45
  ),
  new Vinyl(
    "The Lion For Real",
    "Allen Ginsberg",
    54,
    1989,
    ["spoken word", "jazz"],
    "The Lion For Real",
    "https://www.youtube.com/watch?v=AQropBjNZNQ"
  ),
  new Vinyl(
    "The Medium is the Massage",
    "Marschall McLuhan",
    42,
    1967,
    ["spoken word", "jazz", "electronic", "found sounds"],
    undefined,
    "https://www.youtube.com/watch?v=6SU6Ef30o4E"
  ),
];

export default recordBin;
